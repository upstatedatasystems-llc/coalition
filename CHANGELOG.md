...
- Single-execution concurrency control (`ActiveBuilderRegistry`): tracks active Builder turns per project and epoch in thread-safe memory, rejecting concurrent executions with `CONCURRENT_BUILD_FORBIDDEN` and managing single-ownership session cancellation.
...
- Streamed NDJSON event persistence: sanitizes sensitive credentials/paths with multi-pattern bounded redaction (covering Bearer headers, OpenAI `sk-...`, Google `AIza...`, quoted JSON keys/values, and CLI assignments) ensuring secrets never survive persistent event content or details. Propagates SQLite insertion errors and session status updates as explicit `BuilderError::Database` failures while documenting activity logs as best-effort audit logs.
...
- Calibrated usage & capacity telemetry: authoritative provider-reported token tracking for Antigravity, versioned `ChatGptUsageEstimator` with interactive calibration modal and `calibrate_chatgpt_usage` command, copy-first outbound recording invariant, database-persisted estimator provenance recording calibrated `estimator_version` and exact `chars_per_token` at write time, and honest grounded capacity display.
...
- Comprehensive test suite: 157 unit tests, 2 integration test suites (including 10 Stage 3 end-to-end integration tests verifying hanging-process cancellation and usage calibration provenance), 39 frontend Vitest tests, and zero-quota test double (`fake-agy.cjs`) verifying concurrency, cancellation, calibration, drift rejection, and full lifecycle execution.
