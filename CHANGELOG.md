...
- Common bounded process execution layer (`ProcessRunner`): eliminated stream busy-spinning with guarded `select!` loop, implemented Windows process-tree termination via `taskkill /F /T /PID` to prevent orphaned child processes, added asynchronous stdin streaming with clean EOF closure, routed desktop URL opening and external commands through bounded runners, and added incremental pipe consumption with excess byte draining to prevent deadlock on large streams.
- Memory-bounded output capture: buffers up to 10,000 lines or 10 MB per sync execution with explicit `is_truncated` tracking, bounding text and stderr accumulation to 5 MB each, capping UI streaming logs to 1,000 lines, and strictly enforcing UTF-8 character boundary safety (`truncate_utf8_safe`).
- Pure real `agy` production trust boundary: removed fake-adapter bypass (`use_fake_agy`) from production `StartBuilderTurnPayload`, `list_builder_models`, and production Tauri commands; production Builder strictly resolves official `agy`. Diagnostic proofs are completely isolated via `run_diagnostic_fake_agy_turn` with zero workflow mutation.
- Preflight governance ordering: `start_builder_turn` executes complete preflight checks (repository resolution, restoration reconciliation, frozen snapshot integrity, and contract drift) prior to any workflow state mutation, failing closed with zero state mutation on drift or corruption.
- Single-execution concurrency control & RAII cleanup (`ActiveBuilderRegistry`, `ActiveExecutionGuard`): tracks active Builder turns per project and epoch in thread-safe memory, rejecting concurrent executions with `CONCURRENT_BUILD_FORBIDDEN`, guaranteeing cleanup on all post-registration exit paths (insert failure, persistence failure, task join failure, adapter failure, cancellation, or success), and terminating processes and marking sessions `FAILED` on persistence errors.
- Authoritative cancellation protocol: `cancel_builder_turn` delegates cancellation signaling exclusively to `ActiveBuilderRegistry` without independently mutating operational session state; `BuilderService::start_governed_turn` retains sole authoritative ownership over updating session status to `CANCELLED` after the process tree cleanly terminates. Frontend provides a disabled `⏳ Cancellation requested / terminating…` indicator while termination is pending, keeping running flags intact until final resolution.
- Decoupled `BuilderService`: domain logic decoupled from GUI windowing abstractions into a clean testable service accepting optional event sinks.
- Bounded and sanitized persistence: sanitizes sensitive credentials/paths with multi-pattern bounded redaction (covering Bearer headers, OpenAI `sk-...`, Google `AIza...`, quoted JSON keys/values, and CLI assignments) across all persisted Builder-derived text (`builder_events` content/details, `builder_sessions.response_text`, `builder_sessions.error_message`, and activity messages). Propagates SQLite insertion errors and session status updates as explicit `BuilderError::Database` failures while documenting activity logs as best-effort audit logs.
- Frozen Builder Packet authority: Builder prompt is 100% derived from the Stage 2 frozen `builder-packet.json` contract, epoch ID, and rules; removed arbitrary `followUpPrompt` overrides from production UI, Tauri command payloads, and execution paths.
- Dynamic Antigravity model discovery: queries available models dynamically via `agy models` without hardcoded fallbacks, surfaces discovery errors cleanly in the UI, and supports reasoning effort (`--effort low|medium|high`) and safe mid-conversation model switching.
- Startup session reconciliation: automatically reconciles orphaned `RUNNING` sessions from previous app runs or unexpected crashes to `INTERRUPTED` on startup, logging `BUILDER_SESSION_INTERRUPTED` activity events without touching durable project truth.
- Standardized risk classification & honest evaluation: captures actual tool execution refusal in stderr, classifies risk via `evaluate_tool_risk` (`READ_ONLY`, `MUTATING`, `HIGH_RISK`, `CRITICAL`, or `UNCLASSIFIED_EXTERNAL_ACTION`), records audit history in `builder_permission_history`, and provides actionable guidance without misclassifying generic runtime errors.
- Active-run Icarus immutability: current turn execution mode is immutable once launched; persistent high-visibility banner indicates strictly when an active session was started under Icarus auto-approval (`--dangerously-skip-permissions`).
- Calibrated usage & capacity telemetry: authoritative provider-reported token tracking for Antigravity, versioned `ChatGptUsageEstimator` with interactive calibration modal and `calibrate_chatgpt_usage` command, copy-first outbound recording invariant, typed `TELEMETRY_PERSISTENCE_FAILED` propagation without false clipboard failure claims, database-persisted estimator provenance recording calibrated `estimator_version` and exact `chars_per_token` at write time, and honest grounded capacity display.
- Desktop Builder Control Plane UI: `BuilderControlPlaneView` tab in `ProjectDetailView` with live streaming terminal, packet inspector modal, completion report card, calibration dialog, capacity meters, and permission audit list; removed test-only `useFakeAgy` toggle from production view and cleaned test controls from Diagnostics view.
- Comprehensive test suite: 158 unit tests, 2 integration test suites (including 11 Stage 3 end-to-end integration tests verifying hanging-process cancellation, usage calibration provenance, and injected persistence/session failure cleanup), 39 frontend Vitest tests, and zero-quota test double (`fake-agy.cjs`) verifying concurrency, cancellation, calibration, drift rejection, and full lifecycle execution.
