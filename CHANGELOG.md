...
- Pure real `agy` production trust boundary: removed fake-adapter bypass (`use_fake_agy`) from production `StartBuilderTurnPayload` and Tauri commands; production Builder strictly resolves official `agy`. Diagnostic proofs are completely isolated via `run_diagnostic_fake_agy_turn` with zero workflow mutation.
...
- Streamed NDJSON event persistence: sanitizes sensitive credentials/paths, truncates payloads (32 KB details, 16 KB text deltas), drains background persistence tasks before completion, and bounds historical event queries to chronological order.
...
- Calibrated usage & capacity telemetry: authoritative provider-reported token tracking for Antigravity, versioned `ChatGptUsageEstimator` with interactive calibration modal and `calibrate_chatgpt_usage` command, copy-first outbound recording invariant, database-persisted estimator provenance, and honest grounded capacity display.
- Desktop Builder Control Plane UI: `BuilderControlPlaneView` tab in `ProjectDetailView` with live streaming terminal, packet inspector modal, completion report card, calibration dialog, capacity meters, and permission audit list; removed test-only `useFakeAgy` toggle from production view.
- Comprehensive test suite: 155 unit tests, 2 integration test suites (including 8 Stage 3 end-to-end integration tests), 38 frontend Vitest tests, and zero-quota test double (`fake-agy.cjs`) verifying concurrency, cancellation, calibration, drift rejection, and full lifecycle execution.
