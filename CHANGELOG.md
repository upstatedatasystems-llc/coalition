...
- Common bounded process execution layer (`ProcessRunner`): eliminated stream busy-spinning with guarded `select!` loop, implemented Windows process-tree termination via `taskkill /F /T /PID` to prevent orphaned child processes, added asynchronous stdin streaming with clean EOF closure, and routed production Git, `agy --version`, and `agy models` through bounded sync runner (10 MB buffer, 30s timeout).
- Memory-bounded output capture: buffers up to 10,000 lines or 10 MB per turn with explicit `is_truncated` tracking, bounding text and stderr accumulation to 5 MB each and capping UI streaming logs to 1,000 lines.
...
- Single-execution concurrency control (`ActiveBuilderRegistry`): tracks active Builder turns per project and epoch in thread-safe memory, rejecting concurrent executions with `CONCURRENT_BUILD_FORBIDDEN` and managing per-session cancellation.
- Streamed NDJSON event persistence: sanitizes and records live events into the `builder_events` table (capped at 32 KB per event payload), and restores historical event streams up to 1,000 lines on frontend mount.
...
- Permission architecture & honest evaluation: captures actual tool execution refusal in stderr, classifies risk via `evaluate_tool_risk` (`READ_ONLY`, `MUTATING`, `HIGH_RISK`), records audit history in `builder_permission_history`, and provides actionable guidance without misclassifying generic runtime errors.
- Active-run Icarus immutability: current turn execution mode is immutable once launched; persistent high-visibility banner indicates when an active session was started under Icarus auto-approval (`--dangerously-skip-permissions`).
- Calibrated usage & capacity telemetry: authoritative provider-reported token tracking for Antigravity, versioned `ChatGptUsageEstimator` with interactive calibration modal and `calibrate_chatgpt_usage` command, 5-hour and weekly capacity percentage indicators, and accurate outbound tracking recorded on actual clipboard copy.
...
- Comprehensive test suite: 150 unit tests, 4 integration test binaries, 35 frontend Vitest tests, and zero-quota test double (`fake-agy.cjs`) verifying concurrency, cancellation, calibration, drift rejection, and full lifecycle execution.
